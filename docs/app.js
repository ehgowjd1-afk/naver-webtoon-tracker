...
let APP = null, WEEKDAY = null, GENRE = null, SERIES = null, LOOKUP = {}, WEEKS = [];
let src = "app", sub = "전체", fMode = "all", sMode = "rank", query = "";
let rowsCache = [];

const F_MOVE = [["all","전체"],["up","상승"],["down","하락"]];
const F_APP = [["all","전체"],["up","상승"],["down","하락"],["new","신작·진입"],["streak","연속기록"],["rest","휴재"]];
const F_WT = [["all","전체"],["new","신작"],["rest","휴재"]];
const S_MOVE = [["rank","순위순"],["up","상승폭"],["down","하락폭"]];

const SOURCES = {
  app:     { label:"앱 주간",  subs:()=>[["전체","100"],["여성","50"],["남성","50"]], data:s=>APP&&APP.charts[s], note:()=>`${dot(APP.date)} · 앱 「이번 주 웹툰 랭킹」`, caps:{move:1,streak:1,badge:1,tiles:1,gap:1}, filters:F_APP, sorts:S_MOVE },
  weekday: { label:"요일별",  subs:()=>WEEKDAYS, data:s=>WEEKDAY&&WEEKDAY.days[s], note:()=>`${dot(WEEKDAY.date)} · 요일별 인기순 · 자동수집`, caps:{badge:1}, filters:F_WT, sorts:[] },
  genre:   { label:"장르",    subs:()=>GENRES, data:s=>GENRE&&GENRE.genres[s], note:()=>`${dot(GENRE.date)} · 장르별 인기순 TOP25 · 자동수집`, caps:{badge:1}, filters:F_WT, sorts:[] },
  series:  { label:"시리즈",  subs:()=>SERIES_PERIODS, data:s=>SERIES&&SERIES.periods[s], note:()=>`${dot(SERIES.date)} · 네이버 시리즈 웹툰 · 자동수집`, caps:{move:1,series:1,tiles:1}, filters:F_MOVE, sorts:S_MOVE },
};
...
async function boot(){
  ...
  const results = await Promise.allSettled([
    fetchJSON(`data/${idx.latest}.json`),
    fetchJSON("data/weekday.json"),
    fetchJSON("data/genre.json"),
    fetchJSON("data/series.json"),
    fetchJSON("data/lookup.json"),
  ]);
  APP = results[0].status==="fulfilled" ? results[0].value : { date: idx.latest, charts:{전체:[],여성:[],남성:[]} };
  WEEKDAY = results[1].status==="fulfilled" ? results[1].value : { date:"", days:{} };
  GENRE = results[2].status==="fulfilled" ? results[2].value : { date:"", genres:{} };
  SERIES = results[3].status==="fulfilled" ? results[3].value : { date:"", periods:{} };
  LOOKUP = results[4].status==="fulfilled" ? results[4].value : {};
...
function selectSrc(k){
  ...
  const qEl = document.getElementById("q"); qEl.value = "";
  renderView();
}
...
function renderView(){
  ...
  document.getElementById("meta").innerHTML = `${esc(S.note())} · <b>${esc(subLabel(sub))}</b>`;
...
function curRows(){ const d = SOURCES[src].data(sub); return Array.isArray(d) ? d : []; }

function renderStats(){
  const D = curRows(), caps = SOURCES[src].caps;
  const up=[...D].filter(d=>d.m>0).sort((a,b)=>b.m-a.m), down=[...D].filter(d=>d.m<0).sort((a,b)=>a.m-b.m);
  const news=D.filter(d=>d.b&&(d.b.includes("신작")||d.b.includes("진입"))), streaks=[...D].filter(d=>d.s>0).sort((a,b)=>b.s-a.s);
  const tu=up[0],td=down[0],ts=streaks[0];
  const tiles=[
    {lab:"▲ 최대 상승",cls:"up",big:tu?esc(tu.t):"—",sub:tu?`${tu.r}위 · ▲${tu.m}`:""},
    {lab:"▼ 최대 하락",cls:"down",big:td?esc(td.t):"—",sub:td?`${td.r}위 · ▼${Math.abs(td.m)}`:""},
...
function renderGap(){
  ...
  const rows=both.map(t=>({t,f:fmap.get(t),m:mmap.get(t),gap:mmap.get(t)-fmap.get(t)})).sort((a,b)=>Math.abs(b.gap)-Math.abs(a.gap)).slice(0,5);
...
function enrich(d){
  const lk = LOOKUP[d.t];
  return { id: d.id!=null?d.id:(lk?lk[0]:null), th: d.th||(lk?lk[1]:"") };
}
function renderList(){
  const caps=SOURCES[src].caps, D=curRows();
  let rows=D.filter(d=>{
    ...
    if(fMode==="new") return d.b&&(d.b.includes("신작")||d.b.includes("진입")||d.b.includes("재진입"));
    if(fMode==="streak") return d.s>0;
    if(fMode==="rest") return d.b&&d.b.includes("휴재");
    return true;
  });
  ...
  countEl.textContent=`${rows.length}개 작품`+(fMode!=="all"||query?` (${subLabel(sub)} ${D.length}개 중)`:"");
  if(!rows.length){ board.innerHTML=`<li class="empty">조건에 맞는 작품이 없어요.</li>`; return; }
  board.innerHTML=rows.map((d,i)=>{
    const e=enrich(d);
    const thumb=e.th?`<img class="thumb" loading="lazy" src="${esc(e.th)}" alt="">`:`<div class="thumb ph">🎬</div>`;
...
/* ---------- modal ---------- */
function urlFor(id, title){
  if(SOURCES[src].caps.series) return id?`https://series.naver.com/comic/detail.series?productNo=${id}`:searchUrl(title);
  return id?`https://comic.naver.com/webtoon/list?titleId=${id}`:searchUrl(title);
}
const searchUrl = t => `https://search.naver.com/search.naver?query=${encodeURIComponent(t+" 웹툰")}`;
function openModal(d){
  const e=enrich(d), url=urlFor(e.id,d.t), caps=SOURCES[src].caps;
  ...
  document.getElementById("modalBody").innerHTML=`
    <div class="mtop">
      ${e.th?`<img class="mthumb" src="${esc(e.th)}" alt="">`:`<div class="mthumb"></div>`}
      <div style="min-width:0">
        <div class="mrank">${esc(SOURCES[src].label)} · ${esc(subLabel(sub))} · <b>${d.r}위</b></div>
        <div class="mtitle">${esc(d.t)}</div>
